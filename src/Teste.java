...
public class Teste  extends JPanel {
    Teste() {
        ...
        ((EmployeTempPartiel) exemple[1]).setMontant_heur(20000);
        exemple[1].FeullePresence();
        exemple[1].chequePaie();
        exemple[2] = new EmployeTempPlein("AbdeHalim", "Agent de sécurité");
        ((EmployeTempPlein)exemple[2]) .setMontant_semaine(15000);
        exemple[2].FeullePresence();
        ((EmployeTempPlein)exemple[2]).setPrime(500);
        ...
        String[][] data1 = {{exemple[1].getNom(), exemple[1].getJob(), ((EmployeTempPartiel)exemple[1]).calculPaie() + ""}
                , {exemple[2].getNom(), exemple[2].getJob(), ((EmployeTempPlein)exemple[2]).calculPaie() + ""}};
